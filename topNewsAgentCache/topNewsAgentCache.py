...
CACHE_DB_PATH = os.path.join(os.path.dirname(__file__), "news_cache.db")
...
class CacheManager:
    """
    Manages caching of website content and summaries in a SQLite database.
    """
    
    def __init__(self, db_path=CACHE_DB_PATH, cache_expiry_days=1):
        """
    ...
    def _init_db(self):
        ...
        # Create table for caching website content and summaries
        cursor.execute('''
    ...
    def get_cached_data(self, url):
        """
        Retrieve cached data for a URL if it exists and is not expired.
        
        Args:
            url: The URL to check in the cache
            
        Returns:
            A tuple of (content, summary) if cached, or (None, None) if not
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # Get the cached data
        cursor.execute(
            "SELECT content, summary, timestamp FROM website_cache WHERE url = ?", 
            (url,)
        )
        ...
        if result:
            content, summary, timestamp_str = result
    ...
    def cache_data(self, url, content, summary):
        """
        Cache website content and summary for a URL.
        
        Args:
            url: The URL to cache
            content: The website content to cache
        ...
        cursor.execute(
            "INSERT OR REPLACE INTO website_cache (url, content, summary, timestamp) VALUES (?, ?, ?, ?)",
            (url, content, summary, timestamp)
        )
...
class TopNewsAgentCache:
    ...
    def __init__(self, news_agent: HackerNewsAgent = None, summarizer_agent: WebsiteSummarizerAgent = None, cache_manager: CacheManager = None):
        """
        ...
        Args:
            ...
            cache_manager: A CacheManager instance to use for caching
        """
        ...
        if data_type in ["top_stories", "new_stories", "best_stories"]:
            ...
            for story in stories:
                ...
                if url:
                    ...
                    cached_content, cached_summary = self.cache_manager.get_cached_data(url)
                    
                    if cached_content is not None and cached_summary is not None:
                        print(f"Using cached data for: {title}")
                        content = cached_content
                        summary = cached_summary
                    else:
                        print(f"Fetching and summarizing fresh content for: {title}")
                        ...
                        self.cache_manager.cache_data(url, content, summary)
...
def run_cached_news_workflow():
    """Run the top news workflow with caching"""
    ...
    top_news_agent = TopNewsAgentCache(news_agent, summarizer_agent, cache_manager)
